...
`private`, `cert`. This applies to device `protocols` (every protocol's property map),
device `properties`, device service `properties`, and resource `attributes`. The key is
...
Every list or query tool SHALL cap the number of returned items at the smaller of the
...
the SDK's DNS-rebinding (localhost) protection enabled, and apply a request header read
timeout. Binding to a non-loopback address SHALL log a warning that the MCP endpoint has no
